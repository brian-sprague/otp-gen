...
int main(int argc, char *argv[])
{
	...
	char buffer[1001];
    char* verificationMsgOut = "otp_dec";
    char* inMessage;
    char* outMessage;
    int lineSize;
    int checkSend = -5;
	...
	if (argc < 4)
    {
        fprintf(stderr,"USAGE: %s ciphertextfile keytextfile port\n", argv[0]);
        exit(1);
    }
    ...
    /*************************************************************************/
    // CHECK CIPHER TEXT AND KEY TEXT FILES FOR INVALID CHARS
    /*************************************************************************/

    //Open and check for bad chars in the keytext and ciphertext files
    cipherfp = fopen(argv[1], "r");
    if (cipherfp == NULL) 
    {
    ...
    /*************************************************************************/
    // CHECK THAT KEYTEXT LENGTH IS AT LEAST EQUAL TO cipherTEXT LENGTH
    /*************************************************************************/

    fseek(cipherfp, 0, SEEK_SET);    // Reset the file pointer to the beginning of the file
    fseek(keyfp, 0, SEEK_SET);      // Reset the file pointer to the beginning of the file
    int ctFileSize = getFileSize(cipherfp);
    int ktFileSize = getFileSize(keyfp);
    if (ktFileSize < ctFileSize)
    {
        fclose(keyfp);
        fclose(cipherfp);
        fprintf(stderr, "CLIENT: keytext is shorter than ciphertext. Exiting.\n");
        exit(1);
    }
    ...
    // Connect socket to address, throw error if error
    ...
    if (strncmp(inMessage, "PASS", lineSize) == 0)
    {
        ...
        outMessage = malloc((ctFileSize) * sizeof(char));
        memset(outMessage, '\0', sizeof(outMessage));

        // Fill outMessage with the contents of the plaintext file
        while ((c = fgetc(cipherfp)) != EOF)
        {
        ...
        /*************************************************************************/
        sleep(1);   // Pause to allow server to read the ciphertext
        lineSize = 0;
        fseek(keyfp, 0, SEEK_SET);    // Reset the file pointer to the beginning of the file
        ...
        // Fill outMessage with the contents of the keytext file
        ...
        /*************************************************************************/
        // GET THE PLAIN TEXT FROM THE SERVER
        /*************************************************************************/
